...
def augment_with_smote(X, y, target_samples=1000):
    """
    使用SMOTE方法进行数据扩充
    每类扩充到target_samples个样本
    """
    # 计算当前各类样本数
...
def augment_and_save_with_smote(data_dir, n_splits=5, target_samples=1000):
    """
    使用SMOTE进行数据扩充并保存结果
    保持五折交叉验证结构
    """
    # 创建输出目录
    output_dir = os.path.join(data_dir, 'augmented_smote_data')
    ...
    for fold_idx in range(n_splits):
        ...
        # 数据扩充
        X_aug, y_aug = augment_with_smote(X_train, y_train, target_samples)
...
augment_and_save_with_smote(data_directory, n_splits=5, target_samples=1000)