...
def perform_feature_selection(data_dir, n_splits=5, k_features=100):
    ...
    """
    # 创建输出目录
    output_dir = os.path.join(data_dir, f'selected_features_anova_k{k_features}')
    ...
    for fold_idx in range(n_splits):
        ...
        # 2. 创建ANOVA特征选择器并拟合训练数据
        selector = SelectKBest(score_func=f_classif, k=k_features)
        selector.fit(X_train, y_train)  # ANOVA需要标签信息
        ...
        # 打印特征得分（F值）
        print(f"ANOVA F值: {selector.scores_[selector.get_support(indices=True)]}")
