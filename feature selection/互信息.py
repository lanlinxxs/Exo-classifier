...
def perform_feature_selection(data_dir, n_splits=5, k_features=100):
    ...
    """
    # 创建输出目录
    output_dir = os.path.join(data_dir, f'selected_features_mutual_info_k{k_features}')
    ...
    for fold_idx in range(n_splits):
        ...
        selector.fit(X_train, y_train)  # 互信息需要标签信息
        ...
        # 打印特征得分（互信息值）
        print(f"互信息得分: {selector.scores_[selector.get_support(indices=True)]}")
