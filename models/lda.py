...
from sklearn.discriminant_analysis import LinearDiscriminantAnalysis  # 导入LDA
...
def run_lda_cross_validation(data_dir, n_splits=5):
    """
    执行LDA交叉验证分类
    修改：使用LDA代替朴素贝叶斯
    """
    ...
    for fold_idx in range(n_splits):
        ...
        lda_pipeline = make_pipeline(
            StandardScaler(),
            PCA(n_components=7),  # 你可以试试 100、30 等不同值
            LinearDiscriminantAnalysis(
                solver='svd',
                shrinkage=None,
                priors=[0.1, 0.5, 0.15],  # 更偏的先验
                n_components=2
            )
        )
    ...
    plt.title('LDA - Average Confusion Matrix (values rounded to 5 decimal places)')
...
run_lda_cross_validation(data_directory, n_splits=5)