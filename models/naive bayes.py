...
import numpy as np
import pandas as pd
from sklearn.naive_bayes import MultinomialNB  # 导入MultinomialNB作为贝叶斯分类器
...
def run_bayes_cross_validation(data_dir, n_splits=5):
    """
    执行贝叶斯分类器交叉验证分类
    修改：使用MultinomialNB代替KNN
    """
    # 存储各折结果
    all_reports = []
    confusion_matrices = []
    ...
    for fold_idx in range(n_splits):
        ...
        bayes_classifier = ComplementNB(
            alpha=1500,
            fit_prior=False,
            class_prior=[0.33, 0.33, 0.34]  # 如果是 3 类问题
        )
        ...
        # 验证集预测
        y_pred = bayes_classifier.predict(X_val)
    ...
    plt.title(f'Naive Bayes - Average Confusion Matrix (values rounded to 5 decimal places)')
...
run_bayes_cross_validation(data_directory, n_splits=5)  # 默认使用5折交叉验证
