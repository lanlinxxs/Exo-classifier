...
import numpy as np
import pandas as pd
from sklearn.linear_model import LogisticRegression  # 修改为导入逻辑回归
...
def run_logistic_regression_cross_validation(data_dir, n_splits=5):
    """
    执行逻辑回归交叉验证分类
    修改：使用逻辑回归代替KNN
    """
    ...
    for fold_idx in range(n_splits):
        ...
        lr_pipeline = make_pipeline(
            StandardScaler(),
            LogisticRegression(
                penalty='l1',  # 改用L1正则化（可能不适合某些数据）
                C=1,
                solver='saga',  # 改用saga求解器（可能收敛更慢）
                max_iter=200,  # 更少迭代次数
                multi_class='ovr',  # 改用one-vs-rest策略（可能效果较差）
                random_state=42,
                tol=1e0  # 极大容忍度
            )
        )
    ...
    plt.title('Logistic Regression - Average Confusion Matrix (values rounded to 5 decimal places)')
...
run_logistic_regression_cross_validation(data_directory, n_splits=5)