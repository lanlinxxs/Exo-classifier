...
import numpy as np
import pandas as pd
from sklearn.neighbors import KNeighborsClassifier  # 修改为导入KNN
...
def run_knn_cross_validation(data_dir, n_splits=5, n_neighbors=2):
    """
    执行KNN交叉验证分类
    修改：使用KNN代替决策树
    """
    ...
    for fold_idx in range(n_splits):
        ...
        knn_pipeline = make_pipeline(
            ...
            KNeighborsClassifier(
                n_neighbors=10,  # 过多邻居导致欠拟合
                weights='uniform',  # 忽略距离信息
                algorithm='brute',  # 低效计算
                p=3,  # 曼哈顿距离（假设欧氏距离更合适）
                metric='minkowski'
            )
        )
    ...
    plt.title(f'KNN (k={n_neighbors}) - Average Confusion Matrix (values rounded to 5 decimal places)')
...
run_knn_cross_validation(data_directory, n_splits=5, n_neighbors=5)  # 默认使用5个邻居